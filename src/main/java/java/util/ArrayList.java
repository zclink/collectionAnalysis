        ...
        implements List<E>, RandomAccess, Cloneable, java.io.Serializable
{
     ...
     * 两者区别详见：
     * https://www.cnblogs.com/zh-ch/p/12762619.html
     */
     ...
     * 任何 elementData == DEFAULTCAPACITY_EMPTY_ELEMENTDATA的ArrayList 在添加第一个元素时，都会将elementData扩展为DEFAULT_CAPACITY大小的容量
     * MARK:这里可以看出ArrayList是用数组实现的
     */
    transient Object[] elementData; // non-private to simplify nested class access

    /**
     * ArrayList 真实大小，代表包含了具体多少个元素，elementData的大小只是代表当前的容量
     *
     * @serial
     */
    private int size;
     ...
     * 应用程序可以使用此操作最小化一个ArrayList实例的存储。
     */
     ...
     *  保证最小容量满足需求
     *  MARk:
     *  可以用这个方法初始化ArrayList的容量,在需要添加大量数据时先使用该方法初始化列表容量 可以减少扩容次数，增加性能
     *
     * @param   minCapacity   所需的最小容量
     */
    public void ensureCapacity(int minCapacity) {
        //  除了无参构造的列表 其他全部初始化为默认大小 DEFAULT_CAPACITY = 10
        int minExpand = (elementData != DEFAULTCAPACITY_EMPTY_ELEMENTDATA)
            ...
            : DEFAULT_CAPACITY;
        // 当所需最小容量 minCapacity 大于 默认容量时 minExpand 调用扩容方法
     ...
     * 列表允许最大容量
     * MARK:
     * 但是elementData数组最大可以达到 Integer.MAX_VALUE
     */
     ...
     * 扩容逻辑
     * MARK:
     * 第一步：将容量扩大1.5倍 得到新的容量newCapacity
     * 第二步：与所需最小容量minCapacity对比，若所需最小容量大于newCapacity 则新的容量赋值为minCapacity
     * 第三步：如果新的容量超过了列表最大允许容量 MAX_ARRAY_SIZE 则再判断所需最小容量是否超过了最大允许容量，如果超过了则新容量取值为MAX_ARRAY_SIZE 否则取值为Integer.MAX_VALUE
    ...
    private void grow(int minCapacity) {
        ...
        //如果minCapacity大于最大容量MAX_ARRAY_SIZE，那么新容量newCapacity等于Integer.MAX_VALUE，否者newCapacity等于MAX_ARRAY_SIZE
    ...
    /**
     * 判断所需最小容量 minCapacity 是否超过列表最大容量
     * @param minCapacity
     * @return
     */
     ...
     * 实例的浅拷贝
     * MARK:
     * 也就是说，当列表内存放的是引用对象时，拷贝的对象也是引用,引用的对象改变时 都会改变
     */
    public Object clone() {
        try {
